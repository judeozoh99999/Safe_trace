...
class _TrustedCircleScreenState extends ConsumerState<TrustedCircleScreen> {
  ...
  void initState() {
    super.initState();
    // Guarantee fresh data on screen mount
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ref.invalidate(trustedCircleStreamProvider);
      }
    });
  }
  ...
  Widget _buildSkeletonLoader(bool isDark) {
    return ListView(
      ...
      children: [
        _buildSkeletonSection(isDark, "Active Circle", 2),
        const SizedBox(height: 24),
        _buildSkeletonSection(isDark, "Pending Requests", 1),
        const SizedBox(height: 24),
        _buildSkeletonSection(isDark, "Incoming Requests", 1),
      ],
    );
  }

  Widget _buildSkeletonSection(bool isDark, String title, int count) {
    return Column(
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: circleAsync.when(
          ...
          error: (err, stack) {
            debugPrint('[TRUSTED_CIRCLE_UI] Error: $err');
            return Center(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      onPressed: () => ref.invalidate(trustedCircleStreamProvider),
          ...
          data: (circleData) {
            final activeCircle = circleData.activeCircle;
            final pendingSent = circleData.pendingSent;
            final incomingRequests = circleData.incomingRequests;
